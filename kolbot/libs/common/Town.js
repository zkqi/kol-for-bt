...
var Town = {
	...
	doChores: function () {
		...
		if (stats.firstgame === 1) {
			Pather.walkTo(me.x+2, me.y+2);
			delay(500);
			Precast.weaponSwitch(1);
			delay(500);
			Precast.weaponSwitch(0);
			delay(500);
			DataFile.updateStats("firstgame", 0);
		}
...
MainLoop:
	...
	stash: function (stashGold) {
		...
		var i, result, tier, thw, resulta, string,
			...
			if (stash) {
				if (telekinesis) {
					...
					if ((me.classid === 1 && me.getSkill(54, 1)) || me.getStat(97, 54)) {
						Pather.teleportTo(stash.x,stash.y);
					}
		...
		for (i = 0; !!items && i < items.length; i += 1) {
			if ([18, 41, 76, 77, 78].indexOf(items[i].itemType) === -1 && // Don't drop tomes, keys or potions
				...
				case 0: // Drop item
					...
					if (getUIFlag(0xC) || (Config.PacketShopping && getInteractedNPC() && getInteractedNPC().itemcount > 0 && items[i].getStat(369) == 0)) { // Might as well sell the item if already in shop
						print("clearInventory sell " + items[i].name);
						Misc.itemLogger("Sold", items[i]);
						items[i].sell();
					} else {
						if (items[i].getStat(369) == 0) {
	...
	//Dark-f adding for Barrel on Town
	KickBox: function() {
		var i, wp, dist, vBox, boxId = [3222, 3236, 3256, 3260, 3301];
		wp = getUnit(2, "waypoint");
		for(i=0; i<boxId.length; i++){
			vBox = getUnit(2, getLocaleString(boxId[i]));
			dist = getDistance(vBox, wp);
			if(vBox && vBox.area == me.area && dist <= 30){
				do{
					Misc.openChest(vBox);
					delay(100);
				}while(vBox.getNext());
