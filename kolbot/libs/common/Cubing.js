	...
	buildRecipes: function () {
		...
		for (i = 0; i < Config.Recipes.length; i += 1) {
			...
			case Recipe.Token:
				...
				break;
			case Recipe.Zkqia:
				this.recipes.push({Ingredients: [Config.Recipes[i][1]], Index: Recipe.Zkqia, AlwaysEnabled: true});
...
IngredientLoop:
	...
	validItem: function (unit, recipe) {
		...
		if (recipe.Index === Recipe.Zkqia || recipe.Index === Recipe.Zkqid || recipe.Index === Recipe.Zkqig) {//³é½±¡¢ºÏ³ÉÆìÖÄ¡¢ºÏ³É¼¼ÄÜÊ¯ËéÆ¬
			return true;
		}
		...
		if (recipe.Index === Recipe.Zkqic) {//Ï´ugc ulc usc
			...
			} else if (unit.getStat(378) < 3 && unit.quality === 7) {
				return true;
			}
		...
		if (recipe.Index === Recipe.Zkqib) {//Ò»°ã»ØÊÕ
		...
		if (recipe.Index === Recipe.Zkqif) {//»ØÊÕÇ¿»¯¼¼ÄÜÊ¯
			if (NTIP.CheckItem(unit) === 0) {
				return true;
			}
